...
def _get_uncertain_pt_coords_randomness(mask_coarse_logits, oversample_ratio=3, important_ratio=0.75, cls=0,
    ...
    num_sampled = int(W * H * oversample_ratio)
    ...
    ### 5.
    num_random_points = int(num_uncertain_points * 0.7)
...
def _get_uncertain_point_coords_on_grid(uncertainty_map, rate, batch_size=None):
    """
    args:
    ...
    # print(num_points)
    R, H, W, C = uncertainty_map.shape
    R = batch_size
    if R is None: R = 1
    h_step = 1.0 / (H * 1.0)
    w_step = 1.0 / (W * 1.0)
    num_points = int(H * W * rate)
...
class Get_uncertain_point_coords_on_grid(tf.keras.layers.Layer):
    def __init__(self, rate, batch_size):
        super(Get_uncertain_point_coords_on_grid, self).__init__()
        self.rate = rate
        self.batch_size = batch_size

    def call(self, inputs, **kwargs):
        return _get_uncertain_point_coords_on_grid(inputs, rate=self.rate, batch_size=self.batch_size)


def get_uncertain_point_coords_on_grid(uncertainty_map, rate, batch_size=None):
    return Get_uncertain_point_coords_on_grid(rate=rate, batch_size=batch_size)(uncertainty_map)
